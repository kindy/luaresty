# file from agentzh/test-nginx
package t::Resty::Util;
...
our $NginxPrefix            = $ENV{TEST_NGINX_PREFIX} || '/opt/openresty/nginx';
our $NginxBinary            = $ENV{TEST_NGINX_BINARY} || ($NginxPrefix . '/sbin/nginx');
our $Workers                = 1;
...
our $RestyDir   = File::Spec->catfile($ServRoot, 'luaresty');
...
sub setup_server_root () {
    if (-d $ServRoot) {
        ...
        system("rm -rf $RestyDir > /dev/null") == 0 or
            die "Can't remove $RestyDir";
    ...
    mkdir $RestyDir or
        die "Failed to do mkdir $RestyDir\n";
...
sub _write_files ($$$) {
    ...
    for my $file (@files) {
        ...
        open my $out, ">$baseDir/$fname" or
            die "$name - Cannot open $baseDir/$fname for writing: $!\n";
        print $out $body;
        close $out;

        if ($date) {
            my $cmd = "touch -t '$date' $baseDir/$fname";
            system($cmd) == 0 or
...
sub write_user_files ($) {
    ...
    if ($block->user_files) {
        _write_files($name, $block->user_files, $HtmlDir);
    }
    if ($block->resty_files) {
        _write_files($name, $block->resty_files, $RestyDir);
    }
}
